turtles-own [
   swarmmates         ;; agentset of nearby turtles
  nearest-neighbor   ;; closest one of our  swarmmates
  grouplimit ;; limit of  swarm
  prefcolor ;; color preference
  hatecolor ;; color hated
  densitypref ;; density preference
...
to setup
  ...
  create-turtles population
    [ set shape "turtle"
      ...
      set  swarmmates no-turtles
      set color one-of [brown pink white violet magenta] ;;full color list: red green blue brown pink white violet magenta cyan gray
      set grouplimit random 5
      set prefcolor one-of [brown pink white violet magenta]
      set hatecolor one-of [brown pink white violet magenta]
      set densitypref random 95 + (random (105));;random 15 + (random (25))
  ]
  ask patches[
    set value 0
  ]
...
to density
  ...
  let valview 0
  let valid True
  while [patch-left-and-ahead view 5 != nobody and view != 15]
  [
    set valview (valview + [value] of patch-left-and-ahead view 5)
    set view (view + 1)
  ]
  if valview > densitypref[
    ;; print valview
    set heading (heading + random view + (random (360 - view - view)))]
end
...
to  swarm  ;; turtle procedure
  find-swarmmates
  if any? swarmmates[
    find-nearest-neighbor
    ifelse count swarmmates < grouplimit
    [
      ifelse [color] of nearest-neighbor = hatecolor [separate][cohere]
      ;;cohere
    ]
    [
      ifelse [color] of nearest-neighbor = prefcolor [cohere][separate]
      ;;separate
    ]
  ]
end

to find-swarmmates  ;; turtle procedure
  set swarmmates other turtles in-radius 50
end

to find-nearest-neighbor ;; turtle procedure
  set nearest-neighbor min-one-of  swarmmates [distance myself]
end
...
to-report average-swarmmate-heading  ;; turtle procedure
  ;; We can't just average the heading variables here.
  ;; For example, the average of 1 and 359 should be 0,
  ;; not 180.  So we have to use trigonometry.
  let x-component sum [dx] of  swarmmates
  let y-component sum [dy] of  swarmmates
...
to cohere  ;; turtle procedure
  turn-towards average-swarmmate-heading
  turn-towards average-heading-towards-swarmmates
end

to-report average-heading-towards-swarmmates  ;; turtle procedure
  ;; "towards myself" gives us the heading from the other turtle
  ;; to me, but we want the heading from me to the other turtle,
  ;; so we add 180
  let x-component mean [sin (towards myself + 180)] of  swarmmates
  let y-component mean [cos (towards myself + 180)] of  swarmmates
  ifelse x-component = 0 and y-component = 0
    [ report heading ]
    [ report atan x-component y-component ]
end
...
to-report pnoise
  let x xcor
  let y ycor
  let F 0.5 * (sqrt(3.0) - 1.0)
  let G (3.0 - sqrt(3.0)) / 6.0
  let grad [[1 1 0][-1 1 0][1 -1 0][-1 -1 0]
  ...
  let noise 0.0
	...
	let ii (int(i) mod (length perm / 2))
	let jj (int(j) mod (length perm / 2))
	let gi0 (item (ii + item jj perm) perm) mod 12
	let gi1 (item (ii + i1 + item (jj + j1) perm) perm) mod 12
	let gi2 (item (ii + 1 + item (jj + 1) perm) perm) mod 12

	let tt (0.5 - x0 ^ 2 - y0 ^ 2)
  let gradient item gi0 grad
	if tt > 0
		[set noise tt ^ 4 * (item 0 gradient * x0 + item 1 gradient * y0)]
		
	set tt (0.5 - x1 ^ 2 - y1 ^ 2)
	if tt > 0
		[set gradient item gi1 grad
    set noise (noise + tt ^ 4 * (item 0 gradient * x1 + item 1 gradient * y1))]
		
	set tt (0.5 - x2 ^ 2 - y2 ^ 2)
	if tt > 0
		[set gradient item gi2 grad
    set noise (noise + tt ^ 4 * (item 0 gradient * x2 + item 1 gradient * y2))]

	set noise (noise * 70.0)

  report noise * 40;;random-float -40 + random-float 40 ;;
end
@#$#@#$#@
GRAPHICS-WINDOW
318
